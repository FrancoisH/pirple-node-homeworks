...
server.unifiedServer = function (req, res) {
  ...
  req.on('end', function () {
    ...
    // if one is not found, use the not found one
...
server.init = function() {
  ...
  server.httpServer.listen(config.httpPort, function () {
    console.log(
      '\x1b[36m%s\x1b[0m',
      'The HTTP server is listening on port ' + config.httpPort + ' now'
    );
  });
  ...
  server.httpsServer.listen(config.httpsPort, function () {
    console.log(
      '\x1b[35m%s\x1b[0m',
      'The HTTPS server is listening on port ' + config.httpsPort + ' now'
    );
  });
