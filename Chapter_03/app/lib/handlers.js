            ...
            if (!err && userData) {
              ...
              _data.update('users', phone, userData, function (err) {
                if (!err) {
                  ...
                  console.log(err);
                  callback(500, {'Error': 'Could not update the suer'});
                }
              });
            } else {
  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
        ...
        // Verify that the given token is valid and belongs to the suer who created the check
...
handlers._checks.put = function (data, callback) {
  ...
  if (id) {
    // Check to make sure one or mor optional fields has been set
    if (protocol || url || method || successCodes || timeoutSeconds) {
      _data.read('checks', id, function (err, checkData) {
        if (!err && checkData) {
          var token = typeof(data.headers.token) === 'string' ? data.headers.token : false;
